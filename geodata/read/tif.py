...
# IGMK modules
from utils_igmk.geodata.bounds import convert_bounds_to_gdf
...
def read(
    ...
    if output_format == 'array':
        return read_as_array(
                ...
                Nx=Nx, Ny=Ny, layer=layer,
                )
    elif output_format == 'gdf':
        return read_as_gdf(
                filename, bounds, nan_thresh, scale_factor=scale_factor,
                data_key=data_key, Nx=Nx, Ny=Ny, layer=layer,
                )
    else:
...
def read_as_array(
    ...
    data, meta = sample(data, meta, Nx, Ny)
...
# helper functions                                             #
################################################################
def sample(data, meta, Nx=None, Ny=None):
    shape = np.shape(data)
    size_y, size_x = shape
